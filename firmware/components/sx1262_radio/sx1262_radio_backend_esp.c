...
static bool run_driver_call(sx126x_status_t status, curag_radio_stage_t stage,
                            uint8_t opcode,
                            sx1262_radio_backend_error_t *error) {
...
bool sx1262_radio_backend_initialize(const sx1262_radio_profile_t *profile,
  ...
  if (!run_driver_call(sx126x_reset(&s_backend), CURAG_RADIO_STAGE_RESET, 0U,
                       error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_STANDBY);
  if (!run_driver_call(sx126x_set_standby(&s_backend, SX126X_STANDBY_CFG_RC),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_STANDBY, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_DIO3_TCXO);
  if (!run_driver_call(sx126x_set_dio3_as_tcxo_ctrl(
                           &s_backend,
                           (sx126x_tcxo_ctrl_voltages_t)profile->tcxo_voltage,
                           (uint32_t)profile->tcxo_startup_ms * UINT32_C(64)),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_DIO3_TCXO, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_CLEAR_DEVICE_ERRORS);
  if (!run_driver_call(sx126x_clear_device_errors(&s_backend),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_CLEAR_DEVICE_ERRORS, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND, SX1262_OPCODE_CALIBRATE);
  if (!run_driver_call(sx126x_cal(&s_backend, SX126X_CAL_ALL),
                       CURAG_RADIO_STAGE_WRITE_COMMAND, SX1262_OPCODE_CALIBRATE,
                       error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_CALIBRATE_IMAGE);
  if (!run_driver_call(sx126x_cal_img_in_mhz(&s_backend, 863U, 870U),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_CALIBRATE_IMAGE, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_DIO2_RF_SWITCH);
  if (!run_driver_call(sx126x_set_dio2_as_rf_sw_ctrl(&s_backend, true),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_DIO2_RF_SWITCH, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_REGULATOR_MODE);
  if (!run_driver_call(sx126x_set_reg_mode(&s_backend, SX126X_REG_MODE_DCDC),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_REGULATOR_MODE, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_BUFFER_BASE);
  if (!run_driver_call(sx126x_set_buffer_base_address(&s_backend, 0U, 0U),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_BUFFER_BASE, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_PACKET_TYPE);
  if (!run_driver_call(sx126x_set_pkt_type(&s_backend, SX126X_PKT_TYPE_LORA),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_PACKET_TYPE, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_RF_FREQUENCY);
  if (!run_driver_call(sx126x_set_rf_freq(&s_backend, profile->frequency_hz),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_RF_FREQUENCY, error)) {
    return false;
  }
  ...
  if (!run_driver_call(sx126x_set_lora_mod_params(&s_backend, &modulation),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_MODULATION_PARAMS, error)) {
    return false;
  }
  ...
  if (!run_driver_call(sx126x_set_pa_cfg(&s_backend, &pa),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_PA_CONFIG, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_TX_PARAMS);
  if (!run_driver_call(sx126x_set_tx_params(&s_backend,
                                            profile->tx_params_power_dbm,
                                            SX126X_RAMP_40_US),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
                       SX1262_OPCODE_SET_TX_PARAMS, error)) {
    return false;
  }
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
  ...
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_WRITE_REGISTER);
  if (!run_driver_call(sx126x_cfg_rx_boosted(&s_backend, true),
                       CURAG_RADIO_STAGE_WRITE_COMMAND,
  ...
  if (!run_driver_call(
          ...
          CURAG_RADIO_STAGE_WRITE_COMMAND, SX1262_OPCODE_SET_FALLBACK, error)) {
    return false;
  }
...
bool sx1262_radio_backend_set_standby(sx1262_radio_backend_error_t *error) {
  ...
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_SET_STANDBY);
  return run_driver_call(sx126x_set_standby(&s_backend, SX126X_STANDBY_CFG_RC),
                         CURAG_RADIO_STAGE_WRITE_COMMAND,
                         SX1262_OPCODE_SET_STANDBY, error);
}
...
bool sx1262_radio_backend_set_packet_params(
  ...
  return run_driver_call(sx126x_set_lora_pkt_params(&s_backend, &packet),
                         CURAG_RADIO_STAGE_WRITE_COMMAND,
...
bool sx1262_radio_backend_clear_irq(uint16_t irq_mask,
  ...
  return run_driver_call(sx126x_clear_irq_status(&s_backend, irq_mask),
...
bool sx1262_radio_backend_start_continuous_rx(
  ...
  return run_driver_call(
      sx126x_set_rx_with_timeout_in_rtc_step(&s_backend, SX126X_RX_CONTINUOUS),
  ...
  while (true) {
    ...
    if (xSemaphoreTake(s_backend.dio1_semaphore, wait_ticks) == pdTRUE) {
      ...
      *out_irq_at_us = s_backend.dio1_at_us;
      if (*out_irq_at_us == 0U) {
        *out_irq_at_us = sx1262_radio_backend_monotonic_us();
      }
      return true;
    }
  }
...
bool sx1262_radio_backend_get_irq(uint16_t *out_irq_status,
  ...
  if (!run_driver_call(sx126x_get_irq_status(&s_backend, &irq_status),
                       CURAG_RADIO_STAGE_READ_IRQ, SX1262_OPCODE_GET_IRQ_STATUS,
                       error)) {
    return false;
  }
...
bool sx1262_radio_backend_handle_rx_done(sx1262_radio_backend_error_t *error) {
  ...
  prepare_driver_call(CURAG_RADIO_STAGE_WRITE_COMMAND,
                      SX1262_OPCODE_WRITE_REGISTER);
  return run_driver_call(sx126x_handle_rx_done(&s_backend),
                         CURAG_RADIO_STAGE_WRITE_COMMAND,
                         SX1262_OPCODE_WRITE_REGISTER, error);
}
...
bool sx1262_radio_backend_get_rx_buffer_status(
  ...
  prepare_driver_call(CURAG_RADIO_STAGE_READ_BUFFER,
                      SX1262_OPCODE_GET_RX_BUFFER_STATUS);
  if (!run_driver_call(sx126x_get_rx_buffer_status(&s_backend, &status),
                       CURAG_RADIO_STAGE_READ_BUFFER,
                       SX1262_OPCODE_GET_RX_BUFFER_STATUS, error)) {
    return false;
  }
...
bool sx1262_radio_backend_get_packet_status(
  ...
  if (!run_driver_call(status, CURAG_RADIO_STAGE_READ_PACKET_STATUS,
...
bool sx1262_radio_backend_get_device_errors(
  ...
  prepare_driver_call(CURAG_RADIO_STAGE_READ_COMMAND,
                      SX1262_OPCODE_GET_DEVICE_ERRORS);
  if (!run_driver_call(sx126x_get_device_errors(&s_backend, &device_errors),
                       CURAG_RADIO_STAGE_READ_COMMAND,
                       SX1262_OPCODE_GET_DEVICE_ERRORS, error)) {
    return false;
  }
